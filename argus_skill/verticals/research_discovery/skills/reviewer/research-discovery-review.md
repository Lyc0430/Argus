...
For an initialized Seed Project, reject completion while an automatic request is pending or repair-blocked. Preserve siblings and reject more than five active `probe | select` Bets. For derived children, reject stale or malformed lineage, a relabel-only change, prose-only cross-domain analogy, or a far child without the exact `theory_transfer` role mapping, negative-transfer boundary, and target-domain discriminator. Execution failure may justify repair but never a Rejection Capsule claiming scientific failure.
