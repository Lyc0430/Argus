...
Derived Bets bind their parent through `lineage.source_event_id`, `parent_bet_ids`, positive `generation`, `radius: near | far`, and non-empty `changed_dimensions` drawn only from `mechanism | estimand | prediction`. A far Bet must include the exact `theory_transfer` contract: source domain, source mechanism, target-role mapping, new prediction, negative-transfer boundary, and target-domain probe. An analogy is candidate generation, not evidence; its target-domain probe must still decide the Bet.
