...
When an automatic expansion request exists, plan that exact request without reopening or replacing unrelated siblings. Scientific rejection, novelty collision, or the second distinct information-free result derives exactly one `near` and one `far` route; the first information-free result redesigns the probe. Execution failure schedules bounded repair only, never an Idea pivot. Keep continuation tasks dependency-free from a failed parent so other branches continue, while the project-local five-Bet frontier and host-wide budgets remain authoritative.
